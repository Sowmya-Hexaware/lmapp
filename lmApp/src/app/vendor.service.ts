...
export class VendorService {
  ...
  orderHistory(vendId : number) : Observable<Orders[]> {
    return this.
    http.get("http://localhost:8080/MLP196/api/orderdetail/historyven/"+vendId).
    map((res : Response) => res.json());
}
...
acceptOrReject(orderId : number, vendId : number, status : String) : Observable<string> {
  return this.http.post("http://localhost:8080/MLP196/api/orderdetail/acceptOrRejectOrder/"+orderId + "/" + vendId + "/" +status,null).
  map((res : Response) => res.text());
}
